...
<body>
    <form class="container my-5" style="border: 5px solid;" action="logincheck.php" method="POST">
        ...
        <center><?php
        if (isset($_SESSION['status_sucess'])) 
        {
            ?>
                    <div class="alert alert-success" role="alert">
                        <strong><?php echo $_SESSION['status_sucess']; ?></strong>
                    </div>
            <?php
            unset($_SESSION['status_sucess']);
        }
            ?></center>
        <center><?php
        if (isset($_SESSION['status_incorrect'])) 
        {
            ?>
                    <div class="alert alert-danger" role="alert">
                        <strong><?php echo $_SESSION['status_incorrect']; ?></strong>
                    </div>
            <?php
            unset($_SESSION['status_incorrect']);
        }
            ?></center>
        <center class="my-5">
